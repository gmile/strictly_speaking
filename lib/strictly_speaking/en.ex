  ...
  def say(number, acc, order) when number > 0 do
    ...
    result =
      case {order, div100, div10, rem10} do
        ...
        {0, 0, t, s} ->
          <<elem(@tens, t - 2)::binary, ?\s, elem(@singles, s)::binary>>

        {0, h, 0, 0} ->
          <<elem(@singles, h)::binary, ?\s, "hundred">>

        {0, h, 0, s} ->
          <<elem(@singles, h)::binary, ?\s, "hundred", ?\s, "and", ?\s,
            elem(@singles, s)::binary>>

        {0, h, 1, s} ->
          <<elem(@singles, h)::binary, ?\s, "hundred", ?\s, "and", ?\s, elem(@teens, s)::binary>>
        ...
        {o, 0, 0, s} ->
          <<elem(@singles, s)::binary, ?\s, elem(@bigs, o - 1)::binary>>

        {o, 0, 1, s} ->
          <<elem(@teens, s)::binary, ?\s, elem(@bigs, o - 1)::binary>>

        {o, 0, t, 0} ->
          <<elem(@tens, t - 2)::binary, ?\s, elem(@bigs, o - 1)::binary>>

        {o, 0, t, s} ->
          <<elem(@tens, t - 2)::binary, ?\s, elem(@singles, s)::binary, ?\s,
            elem(@bigs, o - 1)::binary>>

        {o, h, 0, 0} ->
          <<elem(@singles, h)::binary, ?\s, "hundred", ?\s, elem(@bigs, o - 1)::binary>>

        {o, h, 0, s} ->
          <<elem(@singles, h)::binary, ?\s, "hundred", ?\s, "and", ?\s, elem(@singles, s)::binary,
            ?\s, elem(@bigs, o - 1)::binary>>
