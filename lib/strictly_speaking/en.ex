  ...
  @table (
           for n <- 0..999 do
             ...
             case {h, t, s} do
               ...
               {h, 0, 0} -> "#{elem(singles, h)} hundred"
               {h, 0, s} -> "#{elem(singles, h)} hundred and #{elem(singles, s)}"
               {h, 1, s} -> "#{elem(singles, h)} hundred and #{elem(teens, s)}"
               {h, t, 0} -> "#{elem(singles, h)} hundred and #{elem(tens, t - 2)}"
               {h, t, s} -> "#{elem(singles, h)} hundred and #{elem(tens, t - 2)} #{elem(singles, s)}"
             end
           end
           |> List.to_tuple()
         )
